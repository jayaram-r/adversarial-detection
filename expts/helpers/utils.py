...
def load_adversarial_wrapper(i, model_type, adv_attack, max_attack_prop, num_clean_tr, num_clean_te, index_adv=0):
    ...
    if adv_attack != CUSTOM_ATTACK:
        # Different type of attack in the training and test folds for the transfer experiment
        adv_attack_train = adv_attack
        if adv_attack == 'PGD':
            adv_attack_train = 'CW'
        if adv_attack == 'CW':
            adv_attack_train = 'PGD'
        ...
        numpy_save_path = list_all_adversarial_subdirs(model_type, i + 1, adv_attack_train)[0]
        ...
        data_tr_clean, _, _, _, _, _ = load_adversarial_data(
            ...
            norm_type=ATTACK_NORM_MAP.get(adv_attack_train, '2')
        )
        # Adversarial data directories for the test fold
        path_list_all = list_all_adversarial_subdirs(model_type, i + 1, adv_attack)
        path_list_test = path_list_all[:5]
        # Adversarial data directories for the train fold
        path_list_all = list_all_adversarial_subdirs(model_type, i + 1, adv_attack_train)
        path_list_train = path_list_all[:5]

        # Maximum number of adversarial samples from each path in the training fold
        n_paths_tr = len(path_list_train)
        v = int(np.ceil(num_clean_tr / n_paths_tr))
        max_num_tr_list = [v] * n_paths_tr
        max_num_tr_list[-1] = num_clean_tr - (n_paths_tr - 1) * v

        # Maximum number of adversarial samples from each path in the test fold
        n_paths_te = len(path_list_test)
        v = int(np.ceil(max_num_adv / n_paths_te))
        max_num_te_list = [v] * n_paths_te
        max_num_te_list[-1] = max_num_adv - (n_paths_te - 1) * v
