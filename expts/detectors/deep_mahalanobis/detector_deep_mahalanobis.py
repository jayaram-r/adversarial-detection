...
def fit_mahalanobis_scores(model, device, adv_type, net_type, num_labels, outf, train_loader, test_clean_data,
                           test_adv_data, test_noisy_data):
    # numpy arrays to torch tensors
    test_clean_data = torch.from_numpy(test_clean_data).to(device=device, dtype=torch.float)
    test_adv_data = torch.from_numpy(test_adv_data).to(device=device, dtype=torch.float)
    test_noisy_data = torch.from_numpy(test_noisy_data).to(device=device, dtype=torch.float)

    if model.training:
        model.eval()

    # Extract the layer-wise embeddings for a random input with 2 samples
    size_data = test_clean_data.size()
    temp_x = torch.rand(2, size_data[1], size_data[2], size_data[3]).to(device)
    _, temp_list = model.layer_wise_deep_mahalanobis(temp_x)
    num_output = len(temp_list)     # number of layer embeddings
    ...
    for magnitude in m_list:
        ...
        for i in range(num_output):
            ...
            M_in = lib_generation.get_Mahalanobis_score_adv(
                model, device, test_clean_data, num_labels, net_type, sample_mean, precision, i, magnitude
            )
        ...
        for i in range(num_output):
            M_out = lib_generation.get_Mahalanobis_score_adv(
                model, device, test_adv_data, num_labels, net_type, sample_mean, precision, i, magnitude
            )
        ...
        for i in range(num_output):
            M_noisy = lib_generation.get_Mahalanobis_score_adv(
                model, device, test_noisy_data, num_labels, net_type, sample_mean, precision, i, magnitude
            )
            M_noisy = np.asarray(M_noisy, dtype=np.float32)
            if i == 0:
                Mahalanobis_noisy = M_noisy.reshape((M_noisy.shape[0], -1))
            else:
                Mahalanobis_noisy = np.concatenate((Mahalanobis_noisy, M_noisy.reshape((M_noisy.shape[0], -1))),
                                                   axis=1)

        # arrays have shape `(n_samples, n_layers)`
        Mahalanobis_in = np.asarray(Mahalanobis_in, dtype=np.float32)   # clean or in-distribution
        Mahalanobis_out = np.asarray(Mahalanobis_out, dtype=np.float32)     # OOD/Adv
        Mahalanobis_noisy = np.asarray(Mahalanobis_noisy, dtype=np.float32)
        Mahalanobis_pos = np.concatenate((Mahalanobis_in, Mahalanobis_noisy))   # in-distribution
...
def get_mahalanobis_scores(adv_type, dataset, outf):
